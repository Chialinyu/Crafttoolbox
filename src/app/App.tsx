...
import { StitchprintTool } from './components/StitchprintTool';
import { Footer } from './components/Footer';
import { Toaster } from './components/ui/sonner';
import { initGA, trackPageView } from '@/utils/analytics';

type ToolId = 'mosaic-generator' | 'vectorizer-tool' | 'stitchprint-tool';
...
const TOOL_FROM_HASH: Record<string, ToolId> = {
  ...
  stitchprint: 'stitchprint-tool',
  'stitchprint-tool': 'stitchprint-tool',
};
...
function toolToHash(tool: ToolId | null): string {
  ...
  if (tool === 'stitchprint-tool') return '#/stitchprint';
  return '#/';
}
...
function toolPageTitle(tool: ToolId | null): string {
  if (tool === 'mosaic-generator') return 'Mosaic Generator';
  if (tool === 'vectorizer-tool') return 'Vectorizer Tool';
  if (tool === 'stitchprint-tool') return 'Stitchprint';
  return 'Craft ToolBox Home Page';
}
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    trackPageView(pagePath, toolPageTitle(currentTool));
  }, [currentTool]);
  ...
  const renderContent = () => {
    ...
    if (currentTool === 'stitchprint-tool') {
      return <StitchprintTool onBack={goHome} />;
    }
